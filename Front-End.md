...
The **Invested/Not Invested toggle** shows the current `is_invested` value (separate from `start_invested`, which is the historical backtest starting position). Clicking it immediately flips `is_invested` and persists it via `POST /api/config` (fire-and-forget with revert on failure). Toggle is green-tinted for Invested, amber-tinted for Not Invested. When running signals, both `config.start_invested` and `config.is_invested` are passed to `runSignal`. The strategy runs with `start_invested` (cold start); the signal is determined by comparing `is_invested` vs `positions[-1]`: if user is invested and strategy says invested → HOLD; if user is invested and strategy says exit → SELL; if user is not invested and strategy says enter → BUY; otherwise HOLD. The same logic applies in the Current Signal tab (`SignalTab`) — `startInvested` (the dropdown) controls only the historical backtest starting position; `config.is_invested` determines the signal label.
...
0. **Manage Securities** — at the top of the settings sheet. Has two data-update buttons (accent, full-width): **"Update All Securities"** refreshes historical price data for every security serially with a 1500ms pause between each; **"Update Economic Data"** refreshes all three FRED CSVs (credit spread, DGS10, DGS2) with a 1-hour server-side in-memory TTL (no re-fetch within an hour of the last fetch). Both buttons show "Updating..." while in flight; both are disabled while the other is running. Status messages: "All securities updated." / "All securities already up to date." / "Economic data updated." / "Economic data already up to date." — persist until next action. Individual rows each have a **grip handle** (six-dot GripVertical icon) for drag-to-reorder, a refresh icon (re-fetches that security's data from Alpha Vantage), and a trash icon (delete; disabled when only one remains; requires inline confirmation). The Update All button is disabled while any individual update is in flight and vice versa. Dragging a row reorders the list immediately and persists via `POST /api/securities/reorder`; the new order is reflected in all dropdowns. Below the list, an "Add Security" form: Ticker (auto-uppercased), Name, Template (dropdown of existing tickers), and an Add button. Adding auto-fetches the CSV from Alpha Vantage if not already present. After add/remove, the securities list in the header refreshes; if the active ticker was removed, the app switches to the first remaining one.
